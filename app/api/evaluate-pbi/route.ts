...
export async function POST(req: Request) {
  try {
    const { prdContext, breakdownContext, codebaseContext, settings, customPrompts }: 
          { prdContext: string; breakdownContext: string; codebaseContext: string; settings: LLMSettings; customPrompts?: Record<string, string> } = await req.json();
    ...
    const { object: pbiData } = await generateObject({
      ...
      schema: z.object({
        pbis: z.array(z.object({
          description: z.string().describe("A high level summary and scope of the Product Backlog Item"),
          functionalReqs: z.string().describe("Markdown bulleted list of functional requirements for this PBI"),
    ...
    for (const pbi of pbiData.pbis) {
      ...
      const [secRes, archRes, qaRes, lawRes, feRes, beRes] = await Promise.all([
        generateText({
          model: modelObj,
          prompt: createAgentPrompt('SECURITY EXPERT. Focus on security vulnerabilities, authentication flows, data privacy, and potential attack vectors.', customPrompts?.sec_agent)
        }),
        generateText({
          model: modelObj,
          prompt: createAgentPrompt('SYSTEM ARCHITECT. Focus on database design, API structures, state management, file structure, scalability, and existing codebase patterns.', customPrompts?.arch_agent)
        }),
        generateText({
          model: modelObj,
          prompt: createAgentPrompt('QA AUTOMATION ENGINEER. Focus on edge cases, testability, race conditions, and missing acceptance criteria.', customPrompts?.qa_agent)
        }),
        generateText({
          model: modelObj,
          prompt: createAgentPrompt('LEGAL & COMPLIANCE. Focus on GDPR/CCPA implications, terms of service triggers, copyright, and accessibility standards.', customPrompts?.law_agent)
        }),
        generateText({
          model: modelObj,
          prompt: createAgentPrompt('FRONTEND ENGINEER. Focus on UI components, state logic in the browser, CSS/styling, and client-side performance. VERY IMPORTANT: If this PBI is purely backend, explicitly state "Not applicable" and briefly explain why.', customPrompts?.fe_agent)
        }),
        generateText({
          model: modelObj,
          prompt: createAgentPrompt('BACKEND ENGINEER. Focus on API endpoints, database queries, caching layers, business logic, and server performance. VERY IMPORTANT: If this PBI is purely frontend, explicitly state "Not applicable" and briefly explain why.', customPrompts?.be_agent)
        })
      ]);
    ...
    return NextResponse.json({ pbis: evaluatedPBIs });
  } catch (error) {
