...
export default function ReviewPanel({ markdown, pageId }: ReviewPanelProps) {
  const [activeTab, setActiveTab] = useState<'qa' | 'eng' | 'design' | 'notes' | 'breakdown' | 'swarm'>('qa');
  ...
  const [customDesignPriority, setCustomDesignPriority] = useState<'low' | 'medium' | 'high'>('medium');
  ...
  const handleGenerateQuestions = async (type: 'qa' | 'eng' | 'design') => {
    ...
    if (!settings) {
      alert("Please configure your AI Provider Settings first by going back to the home page or via the gear icon.");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        const newQuestions = (data.questions || []).map((q: { text: string; priority?: 'low' | 'medium' | 'high' }, i: number) => ({
          id: `${type}-${Date.now()}-${i}`,
          text: q.text,
          answer: '',
          isAiGenerated: true,
  ...
  const handleGenerateBreakdown = async () => {
    ...
    const formatQ = (list: QuestionObj[], label: string) => {
      ...
      return `### ${label} Decisions:\n` + list.map(q => `**Q:** ${q.text}\n**A:** ${q.answer || 'No answer provided yet.'}`).join('\n\n') + '\n\n';
    };
    ...
    try {
      const res = await fetch('/api/review/breakdown', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ markdown, teamNotes: combinedNotes, settings: getSettings(), customPrompts: getPrompts() }),
      });
      ...
      alert('Error generating breakdown: ' + (e instanceof Error ? e.message : 'An unknown error occurred'));
    } finally {
    ...
    <div className={`review-layout ${activeTab === 'swarm' ? 'review-layout--swarm-active' : ''}`}>
      ...
      <div className="review-right">
        <div className="review-tabs">
          ...
          <button className={`tab-btn ${activeTab === 'eng' ? 'active' : ''}`} onClick={() => setActiveTab('eng')}>
            <Server size={16} /> Eng
          </button>
        ...
        <div className="review-tab-content review-card">
          {activeTab === 'qa' && (
            <div className="tab-pane fade-in">
              ...
              <p className="tab-desc">Generate questions from the perspective of QA to challenge the PRD&apos;s completeness.</p>
              
              {!qaGenerated && qaQuestions.length === 0 ? (
                <button className="btn-primary" onClick={() => handleGenerateQuestions('qa')} disabled={generatingQa}>
                ...
                <div className="question-list">
                      ...
                      <div className="question-content">
                        <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                          <div style={{ display: 'flex', gap: '12px' }}>
                            <span className="question-icon qa-icon"><Bot size={18} /></span>
                            <div>
                               <p style={{ fontWeight: 600, color: 'var(--color-text)', marginBottom: '4px' }}>{q.text}</p>
                               <select 
                                 className={`priority-badge priority-${q.priority}`} 
                                 style={{ border: 'none', outline: 'none', cursor: 'pointer', appearance: 'none' }}
                                 value={q.priority}
                                 onChange={(e) => setQaQuestions(qaQuestions.map(item => item.id === q.id ? { ...item, priority: e.target.value as 'low'|'medium'|'high' } : item))}
                               >
                          ...
                          <button 
                            ...
                            style={{ flexShrink: 0, width: '28px', height: '28px', color: 'var(--color-error)' }}
                            onClick={() => setQaQuestions(qaQuestions.filter(item => item.id !== q.id))}
                        ...
                        <textarea
                          ...
                          value={q.answer}
                          onChange={(e) => setQaQuestions(qaQuestions.map(item => item.id === q.id ? { ...item, answer: e.target.value } : item))}
                        />
                      </div>
                    </div>
                  ))}
                  {!addingCustomQa ? (
                    <button 
                      ...
                      onClick={() => setAddingCustomQa(true)}
                    >
                      + Add Custom Question
                    </button>
                  ) : (
                    <div className="custom-question-form" style={{ background: 'var(--color-surface-sunken)', padding: '16px', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                      <input 
                        type="text" 
                      ...
                      <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                        ...
                        <select 
                           ...
                           onChange={e => setCustomQaPriority(e.target.value as 'low' | 'medium' | 'high')}
                        >
                          <option value="low">Priority: Low</option>
                          <option value="medium">Priority: Medium</option>
                          <option value="high">Priority: High</option>
                        ...
                        <button className="btn-secondary" style={{ padding: '6px 12px' }} onClick={() => { setAddingCustomQa(false); setCustomQaText(''); }}>Cancel</button>
                        <button className="btn-primary" style={{ padding: '6px 12px' }} onClick={() => {
                          if (customQaText.trim()) {
                             setQaQuestions([...qaQuestions, { id: `qa-custom-${Date.now()}`, text: customQaText, answer: '', isAiGenerated: false, priority: customQaPriority }]);
          ...
          {activeTab === 'eng' && (
            <div className="tab-pane fade-in">
              ...
              <p className="tab-desc">Critical technical considerations proposed by the AI to align engineering.</p>
              
              {!engGenerated && engQuestions.length === 0 ? (
                 <button className="btn-primary" onClick={() => handleGenerateQuestions('eng')} disabled={generatingEng}>
                ...
                <div className="question-list">
                  {engQuestions.map((q) => (
                    <div key={q.id} className="question-item interactive">
                      <div className="question-content">
                        <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                          <div style={{ display: 'flex', gap: '12px' }}>
                            <span className="question-icon eng-icon"><Server size={18} /></span>
                            <div>
                              <p style={{ fontWeight: 600, color: 'var(--color-text)', marginBottom: '4px' }}>{q.text}</p>
                              <select 
                                 className={`priority-badge priority-${q.priority}`} 
                                 style={{ border: 'none', outline: 'none', cursor: 'pointer', appearance: 'none' }}
                                 value={q.priority}
                                 onChange={(e) => setEngQuestions(engQuestions.map(item => item.id === q.id ? { ...item, priority: e.target.value as 'low'|'medium'|'high' } : item))}
                               >
                                 <option value="low">Priority: LOW</option>
                                 <option value="medium">Priority: MEDIUM</option>
                                 <option value="high">Priority: HIGH</option>
                          ...
                          <button 
                            className="icon-button" 
                            style={{ flexShrink: 0, width: '28px', height: '28px', color: 'var(--color-error)' }}
                            onClick={() => setEngQuestions(engQuestions.filter(item => item.id !== q.id))}
                            title="Remove Question"
                          >
                            <Trash2 size={16} />
                          </button>
                        ...
                        <textarea
                          ...
                          value={q.answer}
                          onChange={(e) => setEngQuestions(engQuestions.map(item => item.id === q.id ? { ...item, answer: e.target.value } : item))}
                        />
                      </div>
                    </div>
                  ))}
                  {!addingCustomEng ? (
                    <button 
                      className="btn-secondary" 
                      onClick={() => setAddingCustomEng(true)}
                    >
                      + Add Custom Question
                    </button>
                  ) : (
                     <div className="custom-question-form" style={{ background: 'var(--color-surface-sunken)', padding: '16px', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                      ...
                      <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                        ...
                        <select 
                           ...
                           onChange={e => setCustomEngPriority(e.target.value as 'low' | 'medium' | 'high')}
                        >
                          <option value="low">Priority: Low</option>
                          <option value="medium">Priority: Medium</option>
                          <option value="high">Priority: High</option>
                        ...
                        <div style={{ flex: 1 }}></div>
                        <button className="btn-secondary" style={{ padding: '6px 12px' }} onClick={() => { setAddingCustomEng(false); setCustomEngText(''); }}>Cancel</button>
                        <button className="btn-primary" style={{ padding: '6px 12px' }} onClick={() => {
                          if (customEngText.trim()) {
                             setEngQuestions([...engQuestions, { id: `eng-custom-${Date.now()}`, text: customEngText, answer: '', isAiGenerated: false, priority: customEngPriority }]);
          ...
          {activeTab === 'design' && (
            <div className="tab-pane fade-in">
              ...
              <p className="tab-desc">Evaluate user workflows, visual components, interaction patterns, and accessibility needs.</p>
              
              {!designGenerated && designQuestions.length === 0 ? (
                 <button className="btn-primary" onClick={() => handleGenerateQuestions('design')} disabled={generatingDesign}>
                 {generatingDesign ? 'Generating Design Questions...' : 'Generate Design Questions'}
               </button>
             ) : (
                <div className="question-list">
                  {designQuestions.map((q) => (
                    <div key={q.id} className="question-item interactive">
                      <div className="question-content">
                        <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                          <div style={{ display: 'flex', gap: '12px' }}>
                            <span className="question-icon eng-icon" style={{ background: 'var(--color-primary-muted)' }}><Palette size={18} /></span>
                            <div>
                              <p style={{ fontWeight: 600, color: 'var(--color-text)', marginBottom: '4px' }}>{q.text}</p>
                              <select 
                                 className={`priority-badge priority-${q.priority}`} 
                                 style={{ border: 'none', outline: 'none', cursor: 'pointer', appearance: 'none' }}
                                 value={q.priority}
                                 onChange={(e) => setDesignQuestions(designQuestions.map(item => item.id === q.id ? { ...item, priority: e.target.value as 'low'|'medium'|'high' } : item))}
                               >
                                 <option value="low">Priority: LOW</option>
                                 <option value="medium">Priority: MEDIUM</option>
                                 <option value="high">Priority: HIGH</option>
                          ...
                          <button 
                            className="icon-button" 
                            style={{ flexShrink: 0, width: '28px', height: '28px', color: 'var(--color-error)' }}
                            onClick={() => setDesignQuestions(designQuestions.filter(item => item.id !== q.id))}
                            title="Remove Question"
                          >
                            <Trash2 size={16} />
                          </button>
                        ...
                        <textarea
                          ...
                          value={q.answer}
                          onChange={(e) => setDesignQuestions(designQuestions.map(item => item.id === q.id ? { ...item, answer: e.target.value } : item))}
                        />
                      </div>
                    </div>
                  ))}
                  {!addingCustomDesign ? (
                    <button 
                      className="btn-secondary" 
                      onClick={() => setAddingCustomDesign(true)}
                    >
                      + Add Custom Question
                    </button>
                  ) : (
                     <div className="custom-question-form" style={{ background: 'var(--color-surface-sunken)', padding: '16px', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                      ...
                      <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                        ...
                        <select 
                           ...
                           onChange={e => setCustomDesignPriority(e.target.value as 'low' | 'medium' | 'high')}
                        >
                          <option value="low">Priority: Low</option>
                          <option value="medium">Priority: Medium</option>
                          <option value="high">Priority: High</option>
                        ...
                        <div style={{ flex: 1 }}></div>
                        <button className="btn-secondary" style={{ padding: '6px 12px' }} onClick={() => { setAddingCustomDesign(false); setCustomDesignText(''); }}>Cancel</button>
                        <button className="btn-primary" style={{ padding: '6px 12px' }} onClick={() => {
                          if (customDesignText.trim()) {
                             setDesignQuestions([...designQuestions, { id: `design-custom-${Date.now()}`, text: customDesignText, answer: '', isAiGenerated: false, priority: customDesignPriority }]);
          ...
          {activeTab === 'notes' && (
            <div className="tab-pane fade-in">
              ...
              <p className="tab-desc">Jot down any answers to the AI questions, new ideas, or structural notes. This will be fed into the Breakdown step.</p>
              ...
              />
              <p className="form-hint" style={{ marginTop: '12px' }}>Stored temporarily in browser memory for this session.</p>
            </div>
          )}
          ...
          {activeTab === 'breakdown' && (
            <div className="tab-pane fade-in" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
              ...
              <p className="tab-desc">High-level work architecture synthesized from the PRD and team decisions.</p>
              ...
              {!breakdown && !generatingBreakdown ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', flex: 1, minHeight: 0 }}>
                  <div className="markdown-body" style={{ flex: 1, overflowY: 'auto', background: 'var(--color-bg)', padding: '16px', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)' }}>
                    {isEditingBreakdown ? (
                       <textarea
                         className="form-textarea"
                         style={{ width: '100%', height: '100%', minHeight: '300px', resize: 'vertical' }}
                         value={breakdown}
                  ...
                  {breakdown && !generatingBreakdown && (
                    <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', justifyContent: 'flex-end', marginTop: 'auto' }}>
                       <button 
                         className="btn-secondary" 
                       ...
                       <button 
                         ...
                         style={{ backgroundColor: approvedBreakdown ? 'var(--color-success)' : undefined, color: approvedBreakdown ? '#fff' : undefined }}
                       >
                         {approvedBreakdown ? 'Approved ✓' : 'Approve & Proceed'}
                       </button>
                    </div>
                  )}
                  {generatingBreakdown && <p style={{ fontSize: '13px', color: 'var(--color-text-muted)', fontStyle: 'italic', textAlign: 'center' }}>AI is streaming output...</p>}
                </div>
              )}
            </div>
          )}
